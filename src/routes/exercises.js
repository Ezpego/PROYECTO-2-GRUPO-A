...
router.post(
    ...
    wrapWithCatch(async (req, res) => {
        ...
        if (!name || !description || !photo || !muscle_group || !typology) {
            throwErrorAllFieldsRequired();
        }
...
router.patch(
    ...
    wrapWithCatch(async (req, res) => {
        ...
        console.log(typeof req.body.typology);
        const photo = req.files?.photo;
        let image_url_old;
        ...
        if (name) {
            const [[existingExerciseWithSameNameTable]] = await db.execute(
                `SELECT * FROM exercises WHERE name = ? AND id != ? LIMIT  1`,
                [name, exerciseId]
            );
            if (existingExerciseWithSameNameTable) {
                throwErrorNameExerciseExist();
            }
        }
